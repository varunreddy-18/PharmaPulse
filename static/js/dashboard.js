...
function fetchPharmacyData() {
  ...
  fetch(`/pharmacy/${id}/summary`)
    ...
    .then(data => {
      ...
      if (data.error) {
        ...
        document.getElementById('pharmacyNameLabel').textContent = '';
        return;
      }

      document.getElementById('pharmacySummary').innerHTML = `
        <h3>Pharmacy Summary</h3>
        <p><strong>Name:</strong> ${data.pharmacy_name}</p>
  ...
  fetchPharmacyMonthlySales(id);
}

let pharmacyMonthlyChart;

function fetchPharmacyMonthlySales(pharmacyId) {
  fetch(`/pharmacy/${pharmacyId}/monthly-sales`)
    .then(res => res.json())
    .then(data => {
      const labels = data.map(row => row.month);
      const values = data.map(row => row.monthly_sales);
      const ctx = document.getElementById('pharmacyMonthlyChart').getContext('2d');
      if (pharmacyMonthlyChart) pharmacyMonthlyChart.destroy();
      pharmacyMonthlyChart = new Chart(ctx, {
        type: 'bar',
        data: {
          labels: labels,
          datasets: [{
    ...
    .catch(error => {
      console.error('❌ Error loading pharmacy monthly sales:', error);
    });
}
